...
- Lock acquire/release — timeout/retry loop, `FileExistsError` handling, `LockHandle` with an ownership token checked on release (no stale-lock breaking in v1, no time-based staleness anywhere)
- `atomic_write_json` — mesh-root path confinement, temp-file write (collision-resistant naming via `mkstemp`), explicit flush+fsync (+best-effort parent-dir fsync) before `os.replace`, cleanup-on-exception including non-serializable-payload failures, error wrapping
...
- Inbox claim-then-process — atomic claim via `mkdir` + rename into `.processing/<claim_id>/` + sidecar write, delete-after-success, malformed JSON quarantined (not left in place), tolerating concurrent-claim races and all three orphan shapes (empty claim dir, message-without-sidecar, message-with-sidecar), documented best-effort-only ordering
- `recover_processing` — reports/requeues/quarantines claims by age threshold (heuristic only) or by explicit `claim_ids` override; never invoked automatically
...
- **Lock ownership token**: `acquire_lock` tests can directly overwrite `<lock_dir>/owner.token` on disk between acquire and release to simulate "someone else's `mkdir` reacquired this lock name" without needing real multi-process coordination.
...
3. `test_coordinator_locks.py` — `acquire_lock` returns a `LockHandle` (truthy, carries `lock_name` + an opaque token) on success and `None` on timeout; on successful `mkdir`, an `owner.token` file is written inside the lock dir before the handle is returned; `release_lock(handle)` reads `owner.token` and removes the lock dir only if it matches the handle's token; releasing when the on-disk token doesn't match (simulating an external removal + reacquire by someone else) is a silent no-op that does **not** remove the lock — test this by manually swapping the token file between acquire and release; releasing an already-released handle a second time is also a no-op (deliberate — see "Resolved after third adversarial review" on why this isn't a raise); timeout returns `None` with no real sleep; retry-then-succeed via mocked `FileExistsError` once; two-coordinator contention; thread-pool exactly-one-winner race (exactly one thread gets a non-`None` handle); `lock_name` validated via `validate_name` before any `mkdir` attempt.
4. `test_coordinator_atomic_write.py` — writes target content; creates missing parent dirs; rejects a `target_file_path` that resolves outside `mesh_root` (absolute path elsewhere, `..` traversal) with `ValueError`, before any file is touched; temp file uses a collision-resistant name (`tempfile.mkstemp`, not PID-based) so two concurrent writers to the same target never share a temp path; full write sequence is `mkstemp` → write → `flush()` → `os.fsync(fd)` → `close()` → `os.replace()` → best-effort parent-directory fsync that swallows `OSError` (test spies on both fsync calls, and separately confirms a parent-dir-fsync `OSError` doesn't propagate or affect the completed write); descriptor is closed even when a later step fails; cleans up temp file and raises `IOError` on `os.replace` failure; leaves existing target untouched if the temp-file write itself fails; passing a non-JSON-serializable payload (e.g. a `set()`) raises, leaves no temp file, and leaves any existing target untouched.
...
### `inbox.py`
7. `test_inbox_scan.py` — empty inbox; claims a message via `mkdir(.processing/<claim_id>/)` (always succeeds — not the exclusivity point) then `os.rename` of the original message into that dir as `<claim_id>/<original_filename>` (**this rename is the exclusivity point** — two concurrent scans racing the same source message: exactly one rename succeeds, the other gets `FileNotFoundError` and moves on, its now-empty claim dir removed), then writes `<claim_id>/<original_filename>.claim.json` (`claimant_agent_id`, `claimed_at`); reads/deletes the claimed message + sidecar + claim dir on success; sorted-by-filename processing order (not mtime) — asserted only as "best-effort filename order, no cross-machine causal guarantee"; ignores stray `.tmp_*`/hidden files and anything already in `.processing/`; malformed JSON is quarantined — renamed into `inbox/.invalid/` rather than left in place, reported in `InboxScanResult.skipped`; tolerates `Path.unlink`/rename raising `FileNotFoundError` from a concurrent scan.
8. `test_inbox_recovery.py` — `recover_processing(mesh_root, agent_id, older_than_seconds=None, claim_ids=None, action=None)` is a no-op when called with no stale/matching claims; three orphan shapes handled explicitly: (a) an empty `.processing/<claim_id>/` (crash right after `mkdir`, before rename) is simply removed, nothing to report; (b) a claim dir with the message file but no `.claim.json` sidecar (crash after rename, before metadata write) is reported as an orphaned claim, age-checked via the message file's own mtime; (c) a normal claim (message + sidecar) is age-checked via the sidecar's `claimed_at`. Age-based selection (`older_than_seconds`) is documented and tested as a heuristic only; passing explicit `claim_ids` instead recovers exactly those claims regardless of age (the clock-skew-proof override). `action=None` is dry-run/report-only; `action="requeue"` moves the message back to the inbox root; `action="quarantine"` moves it to `.invalid/`; a claim not selected (too young, or not in `claim_ids`) is left untouched under any action; never called by `scan_and_clear_inbox` itself — this is exclusively an explicit, operator-invoked utility.
...
- Manual smoke: simulate each of the three crash points by hand (empty `.processing/<claim_id>/`; message present with no sidecar; complete claim), confirm `scan_and_clear_inbox` leaves all three alone, then confirm `recover_processing` handles each correctly (dry-run report, then `requeue`/`quarantine`) both via age threshold and via explicit `claim_ids`.
