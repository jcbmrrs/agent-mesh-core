...
- `send_message(target_agent_id, message_type, payload)` — atomically drops a timestamped JSON message into another agent's `inbox/`; requires the target inbox to actually be a directory (fails closed on a file or symlink there). **Exposed as an MCP tool.**
- `scan_and_clear_inbox` (`inbox.py`) — claims each message into `.processing/<claim_id>/` (`mkdir` with retry-on-collision + rename + `.claim.json` sidecar), then reads/deletes it; quarantines malformed JSON into `.invalid/` instead of leaving it in place; reports ignored files rather than dropping them silently. Ordering is best-effort filename order only. **Exposed as an MCP tool.**
