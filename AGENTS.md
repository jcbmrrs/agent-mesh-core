...
- **Locking is directory-creation based**, not file-based: `os.mkdir()` on a path under `locks/` is atomic on every target OS (Mac/Linux/Windows), so lock acquisition = successful `mkdir`, release = `rmdir`. Never implement locks with a plain lock *file* (`open(...,'x')`) — the design explicitly calls out that only directory creation is guaranteed atomic across the mixed-OS mount. **No stale-lock breaking in v1** — mtime-based staleness is unsafe under SMB clock skew and would need an ownership-token protocol to be safe; that's deferred, not silently built. `acquire_lock` returns a `LockHandle` (not a bare bool) and `release_lock` takes that handle, as a process-local guard against double-release bugs — it does **not** verify cross-process/cross-machine ownership, since that would reintroduce the same staleness complexity just rejected.
...
- **Inbox messages are claimed before processing** (atomic rename + `.claim.json` sidecar into a `.processing/` subdir), not just read-then-deleted, so two scans of the same inbox can't double-process one message. Malformed messages are quarantined (renamed aside), never left in place for a watcher/re-scan to rediscover forever. A crashed/killed claimant leaves its claim in `.processing/` — recoverable only via an explicit, operator-invoked `recover_processing(older_than_seconds, action)` call, never automatically (same reasoning as no automatic stale-lock breaking).
...
- `acquire_lock(lock_name, timeout, retry_interval) -> LockHandle | None` / `release_lock(handle)` — directory-based mutual exclusion under `locks/`. No stale-lock breaking, no cross-process ownership check in v1.
- `atomic_write_json(target_file_path, data)` — `mkstemp` → write → flush → `fsync` → close → `os.replace`, for any JSON in the mesh.
...
- `scan_and_clear_inbox` (`inbox.py`) — claims each message (rename + `.claim.json` sidecar) into `.processing/`, then reads/deletes it; quarantines malformed JSON into `.invalid/` instead of leaving it in place. Ordering is best-effort filename order only.
- `recover_processing(mesh_root, agent_id, older_than_seconds, action)` (`inbox.py`) — operator-invoked only, never automatic; reports, requeues, or quarantines claims left behind by a crashed/killed claimant.
...
- `PLAN_FEEDBACK.md` / `PLAN_FEEDBACK-2.md` (adversarial reviews) ↔ `IMPLEMENTATION_PLAN.md`'s "Resolved after..." sections and this file's invariants — if a future review lands, add its resolutions the same way rather than editing history away.
- The **superseded banners** at the top of `MULTI-AGENT-GUIDE.md`, `PROJECT-SETUP.md`, and `PROMPT.md` ↔ whatever `IMPLEMENTATION_PLAN.md` currently says supersedes them — if the v1 design changes again, update the banners' pointers too, don't just let them go stale.
