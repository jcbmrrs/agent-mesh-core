...
> **Superseded — historical draft, do not implement as-is.** The `deploy.sh` below (flat `cp` of `src/agent_core.py`, bash `[ -f ... ]` existence check for `local_rules.json`) and the repo layout it assumes are both superseded. Real source is the `src/agent_mesh_core/` uv package (not a flat script); the overwrite guard lives in tested Python (`rules_template.write_local_rules_template`'s `force` flag), not bash; and `deploy.sh`'s actual, narrower job is defined in `../IMPLEMENTATION_PLAN.md`'s "Resolved after second adversarial review" section. **Doubly superseded as of the MCP-server pivot decision (2026-07-23):** "multi-machine sync" below assumed every machine mounts the SMB share and runs `deploy.sh`/the code locally; now only the Mac mini ever touches the mesh root (via its own git checkout) and runs `deploy.sh` — other machines reach it only through an MCP server (or HTTP wrapper), never a mount. See `../PROBLEM_STATEMENT.md`. Read this file for the *why* (clean context windows, revertible logic) — the mechanics are in `../IMPLEMENTATION_PLAN.md`.
